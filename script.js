 // Tableau des cartes Pokémon avec le détails dans l'ordre
        const details = [
            ...
            { name: "Mathilde", image: "image/mathilde.png",gas: false, fille: true,  anniv: "12/04" ,atk: "Citation : <<Ce n'est pas parce qu'en hiver on dit /'fermez la porte qu'il fait froid dehors'/>>", def:"Atouts : Gentillesse ++",aura: 300000000,description: "Le taux de gentillesse dépase les compteurs."},
...
global_button.addEventListener("click", function () {
    let elementsAMasquer = document.querySelectorAll(".annive, .citation, .auras, .auraa, .atouts, .descr");
     ...
     elementsAMasquer.forEach(element => {
